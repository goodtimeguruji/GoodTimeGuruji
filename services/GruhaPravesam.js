...
function getFilteredNakshatra(startItem) {
  ...
  const secondNakshatraList = [
    ...
    "Anuradha", "Uttara Ashadha", "Shravan", "Shatabhisha",
...
async function getAuspiciousTimeWindow(dateStr, userNakshatra, userRasi, lat, lon, tzone, place) {
  ...
  const tarabalamList =
    balam.tarabalam;
  console.log("Chandrabalam:", chandrabalamList);
  console.log("Tarabalam:", tarabalamList);
...
export default async function runAuspiciousCheckAcrossDatesGruhaPravesam(fromDateStr, toDateStr, userNakshatra, userRasi, lat, lon, tzone, place) {
