...
function getFilteredNakshatra(startItem) {
  ...
  const secondNakshatraList = [
    "Mrigashira",
    "Pushya", "Uttara Phalguni", "Hasta", "Chitra", "Anuradha", "Uttara Ashadha", "Shravan", "Dhanishta", "Shatabhisha",
    "Uttara Bhadrapada"
  ];
...
async function getAuspiciousTimeWindow(dateStr, userNakshatra, userRasi, lat, lon, tzone, place) {
  ...
  const disallowedTithis = ["Pratipada", "Chaturthi","Navami", "Chaturdashi", "Purnima","Amavasya"];
  const disallowedYogas = ["Vyaghata", "Vishkumbha", "Parigha", "Shoola", "Ganda", "Vyatipaata", "Vajra", "Sula", "Vaidhriti"];
  const disallowedKaranas = ["Vishti", "Chatushpada", "Nagava", "Kimstughna", "Shakuni"];
...
export default async function runAuspiciousCheckAcrossDatesInvestment(fromDateStr, toDateStr, userNakshatra, userRasi, lat, lon, tzone, place) {
