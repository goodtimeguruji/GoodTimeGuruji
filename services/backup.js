...
export default async function runAuspiciousCheckAcrossDatesAksharaaramba(fromDateStr, toDateStr, userNakshatra, userRasi, lat, lon, tzone, place) {
...
const API_KEY = "a3a1ab378702c90ccc523c59a888f28b";
const AUTH_TOKEN = "Bearer eyJ0eXAiOiJKV1QiLCJhbGciOiJIUzI1NiJ9.eyJpc3MiOiJodHRwczovL2RpdmluZWFwaS5jb20vcmVnZW5lcmF0ZS1hcGkta2V5cyIsImlhdCI6MTc0ODA5NTgzOSwibmJmIjoxNzQ4MDk1ODM5LCJqdGkiOiI3OFNZRjI2aThSYk9JT1hoIiwic3ViIjoiMzY0NiIsInBydiI6ImU2ZTY0YmIwYjYxMjZkNzNjNmI5N2FmYzNiNDY0ZDk4NWY0NmM5ZDcifQ.2rq14SoOpQocVpJmISJeB2amXpudBPHGHdhR123zPrc";

async function getPanchangDetailsForDate(dateStr, lat, lon, tzone, place) {
  const date = new Date(dateStr);
  const baseDateStr = date.toISOString().split("T")[0];
  ...
  const tithis = (data.tithis || [])
    .filter(t => new Date(t.start_time) <= endOfDay)
    .map(t => ({
      date: baseDateStr,
      tithi: t.tithi,
      start_time: t.start_time.replace(" ", "T"),
      end_time:
        new Date(t.end_time) > endOfDay
          ? `${baseDateStr}T23:59:59`
          : t.end_time.replace(" ", "T")
    }));

  const yogas = (data.yogas || [])
    .filter(y => new Date(y.start_time) <= endOfDay)
    .map(y => ({
      date: baseDateStr,
      yoga: y.yoga_name,
      start_time: y.start_time.replace(" ", "T"),
      end_time:
        new Date(y.end_time) > endOfDay
          ? `${baseDateStr}T23:59:59`
          : y.end_time.replace(" ", "T")
    }));

  const karanas = (data.karnas || [])
    .filter(k => new Date(k.start_time) <= endOfDay)
    .map(k => ({
      date: baseDateStr,
      karana: k.karana_name,
      start_time: k.start_time.replace(" ", "T"),
      end_time:
        new Date(k.end_time) > endOfDay
          ? `${baseDateStr}T23:59:59`
          : k.end_time.replace(" ", "T")
    }));
...
async function getBalamTimings(dateStr, nakshatra, rasi, lat, lon, tzone, place) {
  ...
  if (!tarabalamData || !chandrabalamData) {
    return {
      tarabalam: null,
      chandrabalam: null
    };
  ...
  tarabalamData.current =
    (tarabalamData.current || []).map(normalize);

  tarabalamData.next =
    (tarabalamData.next || []).map(normalize);

  nakshatra = normalize(nakshatra);

  return {
    tarabalam: getBalamWindow({
...
async function getAuspiciousTimeWindow(dateStr, userNakshatra, userRasi, lat, lon, tzone, place) {
  ...
  const nakshatraList = panchang?.nakshatras ?? [];
  const tithiList = panchang?.tithis ?? [];
  const yogaList = panchang?.yogas ?? [];
  const karanaList = panchang?.karanas ?? [];
  const waraList = panchang?.waras ?? [];
